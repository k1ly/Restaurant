...
export class DishesService {
  ...
  async create(createDishDto: CreateDishDto) {
    const dish = this.mapper.map(createDishDto, CreateDishDto, Dish);
    dish.category = await this.categoriesService.findById(
      createDishDto.category
    );
    if (!dish.category)
      throw new BadRequestException(
        `Category with id "${createDishDto.category}" doesn't exist!`
      );
    try {
