...
export class CartService {
  ...
  findAll(pageable: Pageable, cartDtos: CartDto[]): [CartDto[], number] {
    return [
      cartDtos
        .sort((a, b) =>
          a[pageable.sort] === b[pageable.sort]
            ? 0
            : a[pageable.sort] < b[pageable.sort]
            ? pageable.order === "asc"
              ? -1
        ...
        .slice(pageable.page * pageable.size)
        .slice(0, pageable.size),
      cartDtos.length,
    ];
  }
  ...
  async create(
    ...
    let cartDto = cartDtos.find(
      (cartDto) => cartDto.dish === createCartDto.dish
    );
    if (cartDto) {
      ...
      if (!(await this.dishesService.findById(createCartDto.dish)))
        throw new BadRequestException(
          `Dish with id "${createCartDto.dish}" doesn't exist!`
        );
      cartDto = new CartDto();
  ...
  updateByDish(
    ...
    const cartDto = cartDtos.find((cartDto) => cartDto.dish === dishId);
  ...
  removeByDish(dishId: number, cartDtos: CartDto[]): CartDto[] {
    cartDtos.splice(
      cartDtos.findIndex((cartDto) => cartDto.dish === dishId),
      1
    );
    return cartDtos;
  }
